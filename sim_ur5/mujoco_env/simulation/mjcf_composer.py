...
class MJCFComposer:
    ...
    def attach_robot(self, spec: RobotSpec, namespace: str):
        """
        ...
        mjcf_robot.model = f"{namespace}_{mjcf_robot.model}"  # Assign unique namespace

        # Apply namespace to sensors
        for sensor in mjcf_robot.find_all('sensor'):
            sensor.name = f"{namespace}_{sensor.name}"

        # Apply namespace to actuators
        for actuator in mjcf_robot.find_all('actuator'):
            actuator.name = f"{namespace}_{actuator.name}"

        # Apply namespace to cameras
        for camera in mjcf_robot.find_all('camera'):
            camera.name = f"{namespace}_{camera.name}"
        ...
        # add attachments with namespace
        mjcf_attachments = attach_addon_collection(spec.attachments, mjcf_robot.worldbody)
        for attachment in mjcf_attachments:
            attachment.model = f"{namespace}_{attachment.model}"

        # update stores
        self._robots[spec] = mjcf_robot
        self._mounted_robots[spec] = mjcf_mounted_robot
        self._attachments[spec] = {spec_att: mjcf_att for spec_att, mjcf_att in zip(spec.attachments, mjcf_attachments)}
    ...
    def _attach_robot_with_mount(self, mjcf_robot, spec):
        ...
        # this can be overridden by providing `base_pos` and `base_rot` or an existing `site` in the mount spec.
